    ...
    public static class CreateCommand
    {
        ...
        internal class CommandHandler : ICommandHandler
        {
            ...
            public async Task HandleAsync(SocketSlashCommand slashCommand)
            {
                ...
                _ = slashCommand.RespondAsync(embeds: embeds);

                var stream = await this._httpClient.GetStreamAsync(image.Url);
                var uploadTask = this._mediaRepo.Upload(commodity.ImageKey, image.ContentType, stream, CancellationToken.None);
                this._objectRepo.Upsert(commodity);

                await this._commandBroker.RegisterAllAsync(guild);
            }
